...
export template <typename Param> concept task_io_params = requires(Param param) {
	param.bytes_transferred = 0;
	param.success = false;
};

export template <typename Param>
...
export template <typename T> class task : public std::suspend_always
{
public:
...
export template <typename JobType, typename IOParams> struct async_io_task : public std::suspend_always
{
public:
	job_queue* queue;
	IOParams	params;
...
export template <typename T> class sync : public std::suspend_never
{
public:
