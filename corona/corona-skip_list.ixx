...
export module corona:skip_list;
...
export template <typename KEY, typename VALUE, int SORT_ORDER = 1> class skip_list
{
public:
...
export template <typename KEY, typename VALUE, int SORT_ORDER> std::ostream& operator <<(std::ostream& output, skip_list<KEY, VALUE, SORT_ORDER>& src)
{
	bool space = false;
	for (auto& l : src)
	{
