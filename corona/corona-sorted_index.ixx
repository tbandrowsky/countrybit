...
export module corona:sorted_index;
...
export using index_ref = relative_ptr_type;
...
export template <typename KEY, typename VALUE, int SORT_ORDER = 1> class sorted_index
{
private:
...
export template <typename KEY, typename VALUE, int SORT_ORDER> std::ostream& operator <<(std::ostream& output, sorted_index<KEY, VALUE, SORT_ORDER>& src)
{
	bool space = false;
	for (auto& l : src)
	{
