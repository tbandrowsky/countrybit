...
namespace countrybit
{
	namespace database
	{
		...
		public:
			...
			{
				boxed<object_id_type>::operator =(_src);
				return *this;
			}
		...
		template<typename T> int operator>(const object_id_box& a, const object_id_box& b)
		{
			return compare(a, b) > 0;
		}
		...
		template<typename T> int compare(const object_id_box& a, T& b)
		{
			return compare(a, boxed(b));
		}

		template<typename T> int operator<(const object_id_box& a, T& b)
		{
			return compare(a, b) < 0;
		}
