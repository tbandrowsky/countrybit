...
/*			command_container.push_button(button_id, class_name, [](pushbutton_control& pc) {
	...
	void run_developer_application(HINSTANCE hInstance, LPSTR  lpszCmdParam)
	{
		...
		if (forceWindowed)
		{
			application->runDialog(hInstance, "COUNTRYBIT REVOLUTION", IDI_REVOLUTION, false, application_presentation);
		}
		else
		{
			application->runDialog(hInstance, "COUNTRYBIT REVOLUTION", IDI_REVOLUTION, true, application_presentation);
		}
	}
