...
export module corona.database:float_box;
import :store_box;

export template <typename floating_point>
...
export template<typename T> std::ostream& operator <<(std::ostream& output, fp_box<T>& src)
{
	output << (T)src;
	return output;
}
...
export double distance(double a, double b)
{
	return std::abs(a - b);
}

export int compare(double a, double b)
{
	return (a <=> b)._Value;
}
